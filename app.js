const cityname = document.getElementById('city-name');
const search = document.querySelector('button');
const show = document.querySelector('#temperature');
const img = document.createElement('img');
const forcastStyle = document.getElementById('forcast');
const bodyStyle = document.querySelector('body');

// API Key
const Api = '040d1885618d5b4f42d9de24a810a32b';
let city;
let GlobalWeather;

// Sumit button
search.addEventListener('click', () => {
    city = cityname.value;
    getWeather();
});

// Converting kelvin to celcius
const ConvertToDegree = (value) => {
    return `${Math.floor(value - 273.15)} C , ${city}, `;
}


async function getWeather() {
    try {
        const response = await fetch(`https://api.openweathermap.org/data/2.5/weather?q=${city}&appid=${Api}`, { mode: 'cors' });
        const ConvertJson = await response.json(); console.log(ConvertJson);

        const TempInKelvin = ConvertJson.main.temp;  // adresss of temperature in json
        const LocalWeather = ConvertJson.weather[0].main;  //weather main address in json
        const CountryName = ConvertJson.sys.country;   // country name address in json
        GlobalWeather = LocalWeather;

        const store = ConvertToDegree(TempInKelvin);

        // styling body
        bodyStyle.style.backgroundColor = '#efc050';
        forcastStyle.style.border = '4px solid white';

        Climate();

        // displaying country name
        show.innerHTML = store + CountryName;

    }
    catch (err) {

        bodyStyle.style.backgroundColor = '#dd4124';
        show.innerHTML = 'Incorrect City Name';
        console.log(err);

    }
}

const Climate = () => {

    switch (GlobalWeather) {

        case 'Clouds':

            img.src = 'https://img.icons8.com/clouds/100/000000/clouds.png';
            document.getElementById('sunny').appendChild(img);
            bodyStyle.style.backgroundImage = "url('images/clouds.jpg')";
            break;


        case 'Rain':

            img.src = 'https://img.icons8.com/color/48/000000/rain--v2.png';
            document.getElementById('sunny').appendChild(img);
            bodyStyle.style.backgroundImage = "url('images/rain.jpg')";
            break;


        case 'Haze':

            img.src = 'data:image/png;base64,';
            document.getElementById('sunny').appendChild(img);
            bodyStyle.style.backgroundImage = "url('images/haze.jpg')";
            break;


        case 'Mist':

            img.src = 'https://img.icons8.com/external-photo3ideastudio-flat-photo3ideastudio/64/000000/external-mist-natural-disaster-photo3ideastudio-flat-photo3ideastudio.png';
            document.getElementById('sunny').appendChild(img);
            bodyStyle.style.backgroundImage = "url('images/Mist.jpg')";
            break;


        case 'Clear':

            img.src = 'https://img.icons8.com/emoji/48/000000/sun-emoji.png';
            document.getElementById('sunny').appendChild(img);
            bodyStyle.style.backgroundImage = "url('images/Clear.jpg')";
            break;


        default:

            img.src = 'https://img.icons8.com/emoji/48/000000/sun-emoji.png';
            document.getElementById('sunny').appendChild(img);
            bodyStyle.style.backgroundImage = "url('images/default.png')";
            break;


    }

}


